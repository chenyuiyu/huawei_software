...
public class FindNextTarget {
    ...
    public int findTarget(Robot r, PlatForm[] p) {
        if (!r.getStatus())
            return findTargetForBuy(r, p);// 为0、1号机器人寻找下一个目标
        ...
        public int compare(PlatForm p1, PlatForm p2) {
            double p1w = this.a * Util.getDistance(p1.getPosition(), curRobotposition)
                    + 20.0 * this.b * Math.abs(Util.getVectorAngle(
                            Util.getVectorBetweenPoints(curRobotposition, p1.getPosition()), curVector));// 权值函数，越小越好

            double p2w = this.a * Util.getDistance(p2.getPosition(), curRobotposition)
                    + 20.0 * this.b * Math.abs(Util.getVectorAngle(
                            Util.getVectorBetweenPoints(curRobotposition, p2.getPosition()), curVector));// 权值函数，越小越好
            if (p1w < p2w)
                return -1;
            return 1;
        }
