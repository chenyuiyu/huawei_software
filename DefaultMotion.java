...
public class DefaultMotion implements MoveType {
     ...
     * @return 当前机器人的指令序列
     */
    public List<Order> Move(Robot curR, List<PlatForm> platFormList, List<List<PlatForm>> labelPlatforms,
        ...
        if (curR.getTargetPlatFormIndex() == -1) {
            ...
            if (platformIdForBuy >= 0) {
                double dis = Util.getDistance(curR.getPosition(), platFormList.get(platformIdForBuy).getPosition());// 距离
        ...
        if (curR.getNearByPlatFormId() == curR.getTargetPlatFormIndex()) { // 靠近目标平台
            ...
            if (!curR.getStatus() && target.HasProduct()) {
                ...
                if (nextTargetPlatForm >= 0) {
                    double dis = Util.getDistance(curR.getPosition(),
                ...
                if (((target.getMateriaStatus() | (1 << index)) >> 1) == (target.getPlatFormType()
                        .getNeededMateria() >> 1)) {
