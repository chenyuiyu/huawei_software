    ...
    private static boolean readFlame(Robot[] rl, PlatForm[] pl, int[] ipc) {
        ...
        while (inStream.hasNextLine()) {
            ...
            if (pcount < k) {
                ...
                if(type == PlatFormType.FOUR) {
                    if(p.getMateriaStatusByIndex(1))curItemPlaceCount[1]++;
                    if(p.getMateriaStatusByIndex(2))curItemPlaceCount[2]++;
                } else if(type == PlatFormType.FIVE) {
                    if(p.getMateriaStatusByIndex(1))curItemPlaceCount[1]++;
                    if(p.getMateriaStatusByIndex(3))curItemPlaceCount[3]++;
                } else if(type == PlatFormType.SIX) {
                    if(p.getMateriaStatusByIndex(2))curItemPlaceCount[2]++;
                    if(p.getMateriaStatusByIndex(3))curItemPlaceCount[3]++;
                } else if(type == PlatFormType.SEVEN) {
                    if(p.getMateriaStatusByIndex(4))curItemPlaceCount[4]++;
                    if(p.getMateriaStatusByIndex(5))curItemPlaceCount[5]++;
                    if(p.getMateriaStatusByIndex(6))curItemPlaceCount[6]++;
                }
            } else {
                // 更新机器人状态
                Robot r = rl[rcount++];
        ...
        for(int i = 1; i <= 6; i++) System.out.printf("%d ", ipc[i]);
        System.out.println("]");
        System.out.print("cipc: [ ");
        for(int i = 1; i <= 6; i++) System.out.printf("%d ", curItemPlaceCount[i]);
        System.out.println("]");
