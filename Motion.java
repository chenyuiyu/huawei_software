...
public class Motion {
     ...
     */
    public static List<Order> Move(Robot r, PlatForm[] p) {
        ...
        double diffangel = Util.getVectorAngle(vector1, vector2);// 偏差角度
        ...
        // double accelerateAngleSpeed = 0.8;// 角加速一帧最多增加0.78
        // 计算角速度减速到0的时间
        int temp = 0;
        double newangleSpeed = angleSpeed;
        if (diffangel < Math.PI / 40) {
            newangleSpeed = 0;
        } else if (diffangel < Math.PI / 20) {
            newangleSpeed = anticlockwise * diffangel;
        } else if (diffangel < Math.PI / 10) {
            newangleSpeed = 0.5 * anticlockwise * diffangel;
            temp = 1;
        } else if (diffangel < Math.PI / 5) {
            newangleSpeed += 0.5 * anticlockwise * diffangel / Math.PI;
            newangleSpeed = Math.max(Math.abs(newangleSpeed), Math.PI / 3) * anticlockwise;
            temp = 2;
        } else if (diffangel < Math.PI / 2) {
            newangleSpeed += anticlockwise * diffangel / Math.PI;
            newangleSpeed = Math.max(Math.abs(newangleSpeed), Math.PI / 2) * anticlockwise;
            temp = 3;
        } else {
